...
public class Evento {
	...
	public static final int EVENT_TERMINE_RITIRO_ISCR_ROW=15;
	public static final int EVENT_TOLLERANZA_PARTECIPANTI_ROW=16;
	...
	public void writeEvent() {
		...
		if(titolo.getValore()!=null) ExcelReader.addToColumn(titolo.getValore(), f, column, FIRST_PAGE);
		else ExcelReader.addToColumn(null, f, column, FIRST_PAGE);
		ExcelReader.addToColumn(""+numPartecipanti.getValore(), f, column, FIRST_PAGE);
		ExcelReader.addToColumn(mylib.Utility.DateToString(termIscr.getValore()), f, column, FIRST_PAGE);
		ExcelReader.addToColumn(luogo.getValore(), f, column, FIRST_PAGE);
		ExcelReader.addToColumn(mylib.Utility.DateToString(dataIn.getValore()), f, column, FIRST_PAGE);
		ExcelReader.addToColumn(oraIn.getValore().toString(), f, column, FIRST_PAGE);
		if(durata.getValore()!=null) ExcelReader.addToColumn(durata.getValore().toString(), f, column, FIRST_PAGE);
		else ExcelReader.addToColumn(null, f, column, FIRST_PAGE);
		ExcelReader.addToColumn(""+quota.getValore(), f, column, FIRST_PAGE);
		if(compresoQuota.getValore()!=null) ExcelReader.addToColumn(compresoQuota.getValore(), f, column, FIRST_PAGE);
		else ExcelReader.addToColumn(null, f, column, FIRST_PAGE);
		if(dataFin.getValore()!=null) ExcelReader.addToColumn(mylib.Utility.DateToString(dataFin.getValore()), f, column, FIRST_PAGE);
		else {
			dataFin.setValore(dataIn.getValore());
			ExcelReader.addToColumn(mylib.Utility.DateToString(dataIn.getValore()), f, column, FIRST_PAGE);
		}
		if(oraFin.getValore()!=null) ExcelReader.addToColumn(oraFin.getValore().toString(), f, column, FIRST_PAGE);
		else ExcelReader.addToColumn(null, f, column, FIRST_PAGE);
		if(note.getValore()!=null)ExcelReader.addToColumn(note.getValore(), f, column, FIRST_PAGE);
		else ExcelReader.addToColumn(null, f, column, FIRST_PAGE);
		if(termRitiroIscr.getValore()!=null)ExcelReader.addToColumn((mylib.Utility.DateToString(termRitiroIscr.getValore())), f, column, FIRST_PAGE);
		else {
			termRitiroIscr.setValore(termIscr.getValore());
			ExcelReader.addToColumn(mylib.Utility.DateToString(termIscr.getValore()), f, column, FIRST_PAGE);
		}
		if(tolleranzaPart.getValore()!=null)ExcelReader.addToColumn(""+tolleranzaPart.getValore(), f, column, FIRST_PAGE);
		else {
			tolleranzaPart.setValore(TOLLERANZA_MIN);
			ExcelReader.addToColumn(""+TOLLERANZA_MIN, f, column, FIRST_PAGE);
		}
